...
const JSONEditor = forwardRef<JSONEditorRef, JSONEditorProps>(
  (
                          ...
                          if (isNestedPropCollapsed) {
                            ...
                            nestedPropKeys.forEach(
                              (nestedKey, nestedKeyIndex) => {
                                ...
                                const finalPropValue =
                                  nestedPropValue[nestedKey];
    ...
    // 格式化JSON
    const handleFormat = (): Promise<void> => {
    ...
    useImperativeHandle(ref, () => ({
      getFormattedJSON: () => {
        ...
        // 如果原始内容无效，但有 lastValidJson，则使用它
        if (lastValidJson) {
          try {
      ...
      validateJSON: () => {
        // 优先验证用户输入的原始内容
        const originalValidation = validateJSON(jsonText);
        ...
        // 如果原始内容无效，但有 lastValidJson，则验证格式化后的内容
        if (lastValidJson) {
          try {
            const { text } = formatJSONWithLines(lastValidJson, true);
