...
import { DragSortableItem } from '../Common/index';
...
import { CardWrapperProps, HeaderDataInterface } from './type';
...
const CardWrapper: React.FC<CardWrapperProps> = ({
  ...
  const cleanRequiredFieldBasedOnTarget = useCallback(
    (
      ...
      // 需要处理required字段的组件类型
      if (
        !['input', 'select_static', 'multi_select_static'].includes(
          component.tag,
        )
      ) {
        return component;
            ...
            if (typeof nextKey === 'number') {
              ...
              if (target && target.elements && Array.isArray(target.elements)) {
                ...
                if (target && target.tag === 'form') {
                  ...
                  // 如果当前目标是表单组件，但elements数组为空，需要创建分栏容器
                  if (
                    target &&
                    target.tag === 'form' &&
    ...
    (
      ...
      if (
        ...
        if (draggedItem.isNew) {
          // 新标题组件，使用默认标题数据
          const defaultHeaderData: HeaderDataInterface = {
            title: { content: '主标题' },
  ...
  const cardStyle: React.CSSProperties = useMemo(() => {
    const baseStyle = {
      backgroundColor: '#fff',
      borderRadius: '4px',
      border: isCardSelected ? '1px solid #1890ff' : '1px solid transparent',
